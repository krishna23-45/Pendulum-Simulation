...
void removePathTrace(){
  for (PathPoint* point : trace_path_container) {
      delete point; 
  }
...
void manageTracePath() {
    ...
    if (trace_path_container.size() >= PATH_LENGTH || pendulum.ang_velocity == 0|| isDragging) {
        delete trace_path_container.front(); // Free memory
...
void energyGraph() {
    ...
    if (TTF_WasInit() || TTF_Init() != -1) {
        ...
        if (font) {
            ...
            if (isDragging) {
                renderLabel(renderer, font, " Kinetic: 0", textColor, SCREEN_WIDTH - GRAPH_WIDTH + 50, SCREEN_HEIGHT - GRAPH_HEIGHT - 10);
                renderLabel(renderer, font, " Potential: 0", textColor, SCREEN_WIDTH - GRAPH_WIDTH + 50, SCREEN_HEIGHT - GRAPH_HEIGHT + 10);
                renderLabel(renderer, font, " Total Energy: 0", textColor, SCREEN_WIDTH - GRAPH_WIDTH + 50, SCREEN_HEIGHT - GRAPH_HEIGHT + 30);
            } else {
                // Render labels
                renderLabel(renderer, font, " Kinetic: " + std::to_string(static_cast<int>(maxKinetic)), textColor, SCREEN_WIDTH - GRAPH_WIDTH + 50, SCREEN_HEIGHT - GRAPH_HEIGHT - 10);
                renderLabel(renderer, font, " Potential: " + std::to_string(static_cast<int>(maxPotential)), textColor, SCREEN_WIDTH - GRAPH_WIDTH + 50, SCREEN_HEIGHT - GRAPH_HEIGHT + 10);
                renderLabel(renderer, font, " Total Energy: " + std::to_string(static_cast<int>(maxPotential + maxKinetic)), textColor, SCREEN_WIDTH - GRAPH_WIDTH + 50, SCREEN_HEIGHT - GRAPH_HEIGHT + 30);
            }
            TTF_CloseFont(font);
        } else {
...
void renderGraph() {
    ...
    if (TTF_WasInit() || TTF_Init() != -1) {
        ...
        if (font) {
            SDL_Color textColor = { 255, 255, 255, 255 };
            renderLabel(renderer, font, "Position: " + std::to_string(static_cast<int>(57.2958 * pos)) + " deg", textColor, 40, SCREEN_HEIGHT - GRAPH_HEIGHT - 5);
            // Cleanup
            TTF_CloseFont(font);
        } else {
            std::cerr << "Font could not be loaded! SDL_ttf Error: " << TTF_GetError() << std::endl;
...
void DestroyScreen() {
    // Clean up dynamically allocated PathPoints
    for (PathPoint* point : trace_path_container) {
        delete point;
    }
    trace_path_container.clear();
...
void handleMouseEvents(const SDL_Event& e) {
    ...
    if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_RIGHT) {
        ...
        if (distance < 20) {
            // If right-clicking near the bob, add a small impulse
            pendulum.ang_velocity += (mx < bobX) ? 0.1 : -0.1;
        } else {
            // If right-clicking away from the bob, start dragging
            isDragging = true;
            lastMouseX = mx;
            lastMouseY = my;
        }
